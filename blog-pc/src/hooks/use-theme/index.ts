...
function getTheme() {
	return getLocalCache(BLOG_GLOBAL_CONFIG) ? getLocalCache(BLOG_GLOBAL_CONFIG).theme : LIGHT
}
function getPrimaryColor() {
	return getLocalCache(BLOG_GLOBAL_CONFIG) ? getLocalCache(BLOG_GLOBAL_CONFIG).primaryColor : DEFAULT_PRIMARY_COLOR
}

const theme = ref(getTheme())
const isDark = computed(() => theme.value === DARK)
const curPrimaryColor = ref(getPrimaryColor())

export const useTheme = () => {
	const switchTheme = (_theme?: ThemeType, isanimation: boolean = false) => {
		theme.value = _theme || getTheme()
		// 开启动画且支持该 api
	...
	const setPrimaryColor = (color?: string) => {
		curPrimaryColor.value = color || getPrimaryColor()
