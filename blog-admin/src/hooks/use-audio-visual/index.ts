...
import { BLOG_ADMIN_PRIMARY_COLOR } from '@/constants'
...
export function useAudioVisual(wrapEL: HTMLElement, audioEl: HTMLAudioElement, cvs: HTMLCanvasElement) {
	...
	function setCanvasSize() {
		...
		const _cw = Math.floor((wrapEL.clientWidth * 0.7) / devicePixelRatio)
		const _ch = Math.floor((wrapEL.clientHeight * 0.7) / devicePixelRatio)
	...
	function drawFerventBar() {
		...
		ctx.fillStyle = getLocalCache(BLOG_ADMIN_PRIMARY_COLOR)
		for (let i = 0; i < len; i++) {
